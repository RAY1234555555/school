...
export default function Transcript({
    ...
    const generatePDF = useCallback(() => {
        ...
        if (!element) { console.error("Transcript content element not found!"); setIsGeneratingPdf(false); return; }
        if (buttonContainer) buttonContainer.style.visibility = 'hidden';
        const opt = { margin: 0.75, filename: `transcript_${studentId || 'student'}.pdf`, image: { type: 'jpeg', quality: 0.95 }, html2canvas: { scale: 2, useCORS: true, logging: false, scrollY: -window.scrollY }, jsPDF: { unit: 'in', format: 'letter', orientation: 'portrait' }, pagebreak: { mode: 'avoid-all', before: '.coursework h2, .summary h2, .legend h2' } };
        html2pdf().set(opt).from(element).save().then(() => { console.log('PDF generated successfully.'); setIsGeneratingPdf(false); if (buttonContainer) buttonContainer.style.visibility = 'visible'; }).catch(err => { console.error('Error generating PDF:', err); alert('Could not generate PDF. Check console for details.'); setIsGeneratingPdf(false); if (buttonContainer) buttonContainer.style.visibility = 'visible'; });
    }, [studentId, isGeneratingPdf]);


    // --- **FIXED** Render Logic ---
    if (error) {
        // This is the corrected error display block
        return (
            ...
            <div className="transcript-container" id="transcript-content">
                ...
                <div className="student-info">
                    ...
                    <div className="info-grid">
                        ...
                        <div><strong>Program:</strong> <span>汉语学习项目</span></div> {/* Still static */}
                        <div><strong>Enrollment Status:</strong> <span>Active</span></div> {/* Still static */}
                    </div>
                </div>
                        ...
                        {coursesData.selectedCourses.map((termData, termIdx) => (
                            <div className="term-courses" key={termIdx}>
                                ...
                                <table className="course-table">
                                    <thead>
                                        <tr><th>Course ID</th><th>Course Title</th><th>Grade</th><th>Credits</th></tr>
                                    </thead>
                                    <tbody>
                                        {termData.courses.map((course, courseIdx) => (
                                            <tr key={courseIdx}><td>{course.courseId}</td><td>{course.title}</td><td>{course.grade}</td><td>{course.credit.toFixed(1)}</td></tr>
                                        ))}
                                    </tbody>
                                </table>
                            </div>
                ...
                <div className="barcode">
                     {displaySid !== 'N/A' ? (<svg id="barcode-svg" width="200" height="70"></svg>) : (<p style={{color: 'grey', fontSize: '12px', marginTop: '5px', height: '70px', display:'flex', alignItems:'center', justifyContent:'center'}}>Barcode N/A (ID Missing)</p>)}
                 </div>
            </div>
